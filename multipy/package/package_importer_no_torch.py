...
from typing import Any, BinaryIO, Callable, cast, Dict, List, Optional, Type, Union
...
class PackageImporter(Importer):
    ...
    def __init__(
        ...
        self.zip_file = zip_file_reader_type(file_or_buffer)
        ...
        for fname in self.zip_file.get_all_records():
    ...
    def load_binary(self, package: str, resource: str) -> bytes:
        ...
        return self.zip_file.get_record(path)
    ...
    def file_structure(
        ...
        return _create_directory_from_file_list(
            self.zip_file.get_filename(),
            self.zip_file.get_all_records(),
            include,
            exclude,
        )
    ...
    def python_version(self):
        ...
        return (
            self.zip_file.get_record(python_version_path).decode("utf-8").strip()
            if self.zip_file.has_record(python_version_path)
            else None
        )

    def _read_extern(self):
        return (
            self.zip_file.get_record(".data/extern_modules")
            .decode("utf-8")
        ...
        for atom in name.split("."):
            if not isinstance(cur, _PackageNode) or atom not in cur.children:
                raise ModuleNotFoundError(
                    f'No module named "{name}" in self-contained archive "{self.zip_file.get_filename()}"'
    ...
    def _compile_source(self, fullpath: str, mangled_filename: str):
        source = self.zip_file.get_record(fullpath)
    ...
    def get_source(self, module_name) -> str:
        ...
        return self.zip_file.get_record(demangle(module.__file__)).decode("utf-8")
...
class _PackageResourceReader:
    ...
    def resource_path(self, resource):
        ...
        if self.importer.zip_file.is_directory() and self.importer.zip_file.has_record(
            os.path.join(self.fullname, resource)
        ):
            return os.path.join(
                self.importer.zip_file.get_filename(), self.fullname, resource
            )
        raise FileNotFoundError

    def is_resource(self, name):
        path = self.importer._zipfile_path(self.fullname, name)
        return self.importer.zip_file.has_record(path)
    ...
    def contents(self):
        ...
        files = self.importer.zip_file.get_all_records()
        subdirs_seen = set()
